...
class DodgeSpecialTask(script: DerangedArchaeologistMagicKiller) : Task(script) {
    ...
    private fun isSpecialAttackActive(): Boolean {
        ...
        val overheadText = Npcs.stream().name(Constants.BOSS_NAME).any {
            it.overheadMessage() == Constants.SPECIAL_ATTACK_TEXT
        }
    ...
    private fun findPotentialSafeTiles(playerTile: org.powbot.api.Tile, boss: Npc?, allProjectiles: List<Projectile>): List<org.powbot.api.Tile> {
        ...
        val angleToBoss = if (boss != null && boss.valid()) ScriptUtils.angleBetween(playerTile, boss.tile()) else null
        ...
        return Constants.DODGE_TILES.filter { tile ->
            ...
            val isAwayFromProjectiles = projectileTiles.all { projTile ->
                tile.distanceTo(projTile) > Constants.PROJECTILE_DANGER_DISTANCE
            }
            ...
            val avoidsBossPath = if (angleToBoss != null) {
                val angleToTile = ScriptUtils.angleBetween(playerTile, tile)
    ...
    override fun execute() {
            ...
            if (bestSafeTile != null) {
                ...
                if (Movement.step(bestSafeTile)) {
                    ...
                    val waitResult = Condition.wait({
                        ...
                        val tooClose = currentProjsDuringWait.any {
                            player.tile().distanceTo(it.tile()) <= Constants.PROJECTILE_DANGER_DISTANCE
                        }
        ...
        if (dodgeSuccess) {
            ...
            val isSafeFromProjectiles = finalProjectiles.all {
                player.tile().distanceTo(it.tile()) > Constants.PROJECTILE_DANGER_DISTANCE
            }
            ...
            if (isSafeFromProjectiles) {
                ...
                if (finalBoss != null && finalBoss.valid()) {
                    ScriptUtils.attackBoss(finalBoss, script)
                } else {
