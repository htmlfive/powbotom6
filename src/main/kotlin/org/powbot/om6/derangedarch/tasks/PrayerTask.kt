...
class PrayerTask(script: DerangedArchaeologistMagicKiller) : Task(script) {
    ...
    override fun validate(): Boolean {
        ...
        val shouldRun = prayerNotActive || prayerLow
        
        if (shouldRun) {
            script.logger.debug("Validate OK: Prayer needs attention (active: ${!prayerNotActive}, points: ${Prayer.prayerPoints()})")
        }
